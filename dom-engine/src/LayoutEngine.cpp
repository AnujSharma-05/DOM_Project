...
LayoutStats LayoutEngine::run(Node& root) const {
    LayoutStats stats;
    visit(root, false, 0, 0, stats);
    return stats;
}

void LayoutEngine::visit(Node& node, bool parent_dirty, int parent_abs_x, int parent_abs_y, LayoutStats& stats) const {
